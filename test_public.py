...
if __name__=="__main__":
    ...
    # ==========================================
    # RUN WITH / WITHOUT SPECIFIC OBJECT
    # ==========================================
    # Uncomment the lines below to isolate specific objects or resume a partial run.
    
    # 1. Isolate specific objects for targeted testing:
    # objects = ["sheet_metal"]

    # 2. Drop specific objects (Useful for skipping previously completed classes):
    # objects_to_drop = ["can", "fabric", "sheet_metal"] # Add completed objects here, e.g., ["can", "vial"]
    # objects = [obj for obj in objects if obj not in objects_to_drop]
            ...
            if os.path.exists(f"{results_dir}/metrics_seed={seed}.json"):
                ...
                with open(timeit_file, 'w', newline='') as file:
                    ...
                    for object_name in objects:
                        # Forced the resolution to 448 for all objects
                        model = get_model(args.model_name, 'cuda', smaller_edge_size=448, object_name=object_name)
